...
- Опциональный NLP-слой на spaCy (`ru_core_news_sm`): извлечение города из
  сообщения, чтобы погода отвечала сразу, и POS-фильтр служебных слов для
  снижения ложных срабатываний (`SPACY_ENABLED`, `SPACY_MATCHING`).
...
- Погода: живые данные Open-Meteo (geocoding + forecast), при сбое сети — явно
  помеченная «догадка», при неизвестном городе — переспрос. Если город назван в
  самом вопросе («какая погода в Питере»), уточняющий шаг пропускается, а название
  приводится к начальной форме («Москве» → «Москва»).
...
- Сценарии с уточнением: приём заявки (имя → телефон → направление) и погода
  (город). Общий движок FSM; на любом шаге доступна отмена, а новый вопрос
  (например, «Кто ректор?») корректно прерывает сценарий.
...
backend/
  app/
    ...
    nlp_spacy.py     опциональный spaCy: сущности и POS-фильтр (ленивая загрузка)
    dialog.py        ядро диалога + универсальный FSM сценариев
    weather.py       Open-Meteo: прогноз и «догадка» при сбое
    engine.py        сборка движка, регистрация resolver'ов и экстракторов
  ...
  tools/
    ...
    eval_recognition.py оценка точности распознавания (бенчмарк)
  tests/               pytest
frontend/
  src/               React + TypeScript SPA
docs/                  аналитический обзор и предложения по развитию
...
Если включён `SPACY_ENABLED=true`, бот извлекает город прямо из вопроса и не
переспрашивает:

```text
Вы: Какая погода в Питере?
Бот: Погода в Санкт-Петербурге: +2 °C, облачно, ветер 3 м/с.
```
...
(успех, отмена, повторный запрос, город из сообщения), fallback, погодный сервис
с моком сети, NLP-слой spaCy, сборка базы знаний (`build_kb`) и эндпоинты API.
...
- **Погода** — `app/weather.py` обращается к бесплатному Open-Meteo (без токена).
  ...
  **явно помечает его как догадку**. Если город не найден, бот переспрашивает, а не
  угадывает. При `SPACY_ENABLED=true` город извлекается из самого вопроса.
...
Бенчмарк на наборе тематических и посторонних фраз:

```bash
cd backend
python -m tools.eval_recognition
# с включённым spaCy и POS-фильтром:
SPACY_ENABLED=true SPACY_MATCHING=true python -m tools.eval_recognition
```

Команда печатает точность распознавания, долю ложных срабатываний и среднее
время распознавания. Подробные измерения — в `docs/analytical-review.md`.
...
| `SPACY_ENABLED`       | `true`/`false` — NLP-слой spaCy (сущности, город в погоде) |
| `SPACY_MATCHING`      | `true`/`false` — POS-фильтр служебных слов при распознавании |
| `SPACY_MODEL`         | имя модели spaCy (по умолчанию `ru_core_news_sm`) |
